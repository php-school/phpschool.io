...
<template>
    ...
    <!-- promote alerts -->
    <alert type="error" :message="promoteError ?? 'An error occurred. Please try again later.'" :timeout="4000" v-if="showPromoteError" @close="showPromoteError = false"></alert>
    <alert type="success" :message="promoteSuccess" :timeout="4000" v-if="showPromoteSuccess" @close="showPromoteSuccess = false"></alert>

    <!-- regenerate alerts -->
    <alert type="error" :message="regenerateError ?? 'An error occurred. Please try again later.'" :timeout="4000" v-if="showRegenerateError" @close="showRegenerateError = false"></alert>
    <alert type="success" message="Successfully regenerated workshop feed" :timeout="4000" v-if="showRegenerateSuccess" @close="showRegenerateSuccess = false"></alert>
    ...
    <header class="flex items-center justify-between border-b border-pink-600/30 px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
        ...
        <button @click="regenerateFeed" type="button" class="rounded-md flex-none py-1.5 px-2 text-xs font-medium ring-1 ring-inset text-pink-500 hover:text-pink-600 bg-pink-500/10 ring-pink-500/30 hover:ring-pink-600/30">Regenerate Feed</button>
    </header>
    ...
    <ul role="list" class="divide-y divide-pink-600/30">
        <li v-for="workshop in filteredWorkshops" :key="workshop.id" class="relative flex items-center space-x-4 px-4 py-4 sm:px-6 lg:px-8">
            <div class="min-w-0 flex-auto">
                ...
                <div class="mt-3 flex items-center gap-x-2.5 text-xs leading-5 text-gray-400">
                    <div :class="[types[workshop.type], 'rounded-full flex-none py-0.5 px-2 text-[10px] font-medium ring-1 ring-inset']">{{ workshop.type }}</div>
                    <svg viewBox="0 0 2 2" class="h-0.5 w-0.5 flex-none fill-gray-300">
                        <circle cx="1" cy="1" r="1" />
                    </svg>
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                    <MenuItems class="absolute right-0 z-10 p-2 mt-2 w-48 origin-top-right divide-y divide-gray-700 rounded-md bg-gray-800 shadow-lg ring-1 ring-pink-500/50 focus:outline-none">
                        <MenuItem v-if="workshop.type === 'Community'" v-slot="{ active }">
                            <button @click="confirmPromoteWorkshop(workshop)" :class="[active ? 'p-2 text-white' : 'text-slate-400', 'group flex items-center px-4 py-2 text-sm w-full']">
                                <ArrowUpIcon class="mr-3 h-5 w-5" aria-hidden="true" />
                                Promote to Core<span class="sr-only">, {{ workshop.code }}</span>
                            </button>
                        </MenuItem>
                        <MenuItem v-slot="{ active }">
                            <button @click="confirmDeleteWorkshop(workshop)" :class="[active ? 'p-2 text-white' : 'text-slate-400', 'group flex items-center px-4 py-2 text-sm w-full']">
