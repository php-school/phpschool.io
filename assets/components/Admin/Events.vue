<template>
    ...
    <alert type="success" :message="deleteSuccess" :timeout="4000" v-if="showDeleteSuccess" @close="showDeleteSuccess = false"></alert>

    <!-- event create/update alerts -->
    <alert type="error" :message="eventError !== '' ? eventError : 'An error occurred. Please try again later.'" :timeout="4000" v-if="showEventError" @close="showEventError = false"></alert>
    <alert type="success" :message="eventSuccess" :timeout="4000" v-if="showEventSuccess" @close="showEventSuccess = false"></alert>
    ...
    <header class="flex items-center justify-between border-b border-pink-600/30 px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
        ...
        <button @click="addEvent" type="button" class="rounded-md flex-none py-1.5 px-2 text-xs font-medium ring-1 ring-inset text-pink-500 hover:text-pink-600 bg-pink-500/10 ring-pink-500/30 hover:ring-pink-600/30">Add</button>
    </header>
    ...
    <ul role="list" class="divide-y divide-pink-600/30">
        <li v-for="event in filteredEvents" :key="event.id" class="relative flex items-center space-x-4 px-4 py-4 sm:pr-6 lg:pr-8 ">
            ...
            <div class="min-w-0 flex-auto">
                ...
                <p class="mt-1 line-clamp-1 text-sm leading-6 text-gray-300">{{event.description}}</p>
            </div>
            <Menu as="div" class="relative inline-block text-left">
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                    <MenuItems class="absolute right-0 z-10 p-2 mt-2 w-48 origin-top-right divide-y divide-gray-700 rounded-md bg-gray-800 shadow-lg ring-1 ring-pink-500/50 focus:outline-none">
                        <MenuItem v-slot="{ active }">
                            <button @click="editEvent(event)" :class="[active ? 'p-2 text-white' : 'text-slate-400', 'group flex items-center px-4 py-2 text-sm w-full']">
                                <PencilIcon class="mr-3 h-5 w-5" aria-hidden="true" />
                                Edit<span class="sr-only">, {{ event.name }}</span>
                            </button>
                        </MenuItem>
                        <MenuItem v-slot="{ active }">
                            <button @click="confirmDeleteEvent(event)" :class="[active ? 'p-2 text-white' : 'text-slate-400', 'group flex items-center px-4 py-2 text-sm w-full']">
        ...
        <Dialog as="div" class="relative z-50" @close="showAdd = false">
            ...
            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center">
                    <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                        <DialogPanel class="relative transform overflow-hidden rounded-lg bg-gray-900 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8  w-full md:max-w-2xl sm:p-6">
                            <div class="sm:flex sm:items-start">
                                <form @submit.prevent="saveEvent" class="w-full" enctype="multipart/form-data">
                                    <div class="space-y-12">
                                        <div class="border-b border-white/10 pb-12">
                                            <h2 v-if="form.id" class="text-base font-semibold leading-7 text-white">
                                                Updating event</h2>
                                            <h2 v-else class="text-base font-semibold leading-7 text-white">Creating new
                                                event</h2>
                                            <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                                <div class="col-span-full">
                                                    ...
                                                    <ul v-if="errors.name" class="">
                                                        <li v-for="err in errors.name"
                                                            class="mt-2 text-xs text-red-600">{{ err }}
                                                        </li>
                                                    </ul>
                                                </div>
                                                ...
                                                <div class="col-span-full">
                                                    ...
                                                    <p class="mt-3 text-sm leading-6 text-gray-400">Write a description
                                                        of the event.</p>
                                                    <ul v-if="errors.description" class="">
                                                        <li v-for="err in errors.description"
                                                            class="mt-2 text-xs text-red-600">{{ err }}
                                                        </li>
                                                    </ul>
                                                </div>
                                                ...
                                                <div class="col-span-full">
                                                    <label for="link"
                                                           class="block text-sm font-medium leading-6 text-white">Link</label>
                                                    <div class="mt-2">
                                                        <div class="flex rounded-md bg-white/5 ring-1 ring-inset ring-white/10 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500">
                                                ...
                                                <div class="col-span-full">
                                                    <label for="date"
                                                           class="block text-sm font-medium leading-6 text-white">Date</label>
                                                    <div class="mt-2">
                                                        <div class="flex rounded-md bg-white/5 ring-1 ring-inset ring-white/10 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500">
                                                            <input v-model="form.date" type="datetime-local" name="date"
                                                                   id="date" autocomplete="link"
                                                                   class="flex-1 border-0 bg-transparent py-1.5 pl-1 text-white focus:ring-0 sm:text-sm sm:leading-6"/>
                                                        </div>
                                                    </div>
                                                    <ul v-if="errors.date" class="">
                                                        <li v-for="err in errors.date"
                                                            class="mt-2 text-xs text-red-600">{{ err }}
                                                        </li>
                                                    </ul>
                                                </div>
                                                ...
                                                <div class="col-span-full">
                                                    <label for="venue"
                                                           class="block text-sm font-medium leading-6 text-white">Venue</label>
                                                    <div class="mt-2">
                                                        <textarea v-model="form.venue" id="venue" name="venue" rows="3"
                                                                  class="block w-full rounded-md border-0 bg-white/5 py-1.5 text-white shadow-sm ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm sm:leading-6"/>
                                                    </div>
                                                    <ul v-if="errors.venue" class="">
                                                        <li v-for="err in errors.venue"
                                                            class="mt-2 text-xs text-red-600">{{ err }}
                                                        </li>
                                                    </ul>
                                                </div>
                                                ...
                                                <div class="col-span-full">
                                                    <label for="poster"
                                                           class="block text-sm font-medium leading-6 text-white">Poster</label>

                                                    <div class="mt-2">

                                                        <label for="file-input" class="sr-only">Choose file</label>
                                                        <input type="file" ref="fileInput" @change="changeEventImage" accept="image/*"  name="poster" id="poster"
                                                               class="block w-full border border-gray-200 shadow-sm rounded-lg text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:border-gray-700 text-white file:text-white dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600 file:border-0 bg-white/5 file:bg-white/5 file:me-4 file:py-3 file:px-4 ">
                                                    </div>
                                                    <ul v-if="errors.poster" class="">
                                                        <li v-for="err in errors.poster"
                                                            class="mt-2 text-xs text-red-600">{{ err }}
                                                        </li>
                                                    </ul>
                                                </div>
                            ...
                            <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                <button type="button" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto" @click="saveEvent">
                                    <ArrowPathIcon v-show="loading" class="w-5 h-5 animate-spin"/>
                                    <span v-show="!loading">Save</span>
                                </button>
                                <button type="button" class="mt-3 inline-flex w-full justify-center rounded-md bg-white/10 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-white/20 sm:mt-0 sm:w-auto" ref="cancelButtonRef" @click="showAdd = false">Cancel</button>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
                </div>
