...
<template>
    ...
    <alert
        type="error"
        :message="cacheClearError ?? 'An error occurred. Please try again later.'"
        :timeout="4000"
        :show="showCacheClearError"
        @close="showCacheClearError = false"
    ></alert>
    <alert
        type="success"
        message="Cache was successfully cleared"
        :timeout="4000"
        :show="showCacheClearSuccess"
        @close="showCacheClearSuccess = false"
    ></alert>
    ...
    <div
        class="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-4 py-16 sm:px-6 md:grid-cols-2 lg:px-8 border-b border-pink-600/30"
    >
        <div>
            <h2 class="text-base font-semibold leading-7 text-white">Clear cache</h2>
            <p class="mt-1 text-sm leading-6 text-gray-400">
                Clear the full page and Redis cache. This includes Doctrine and HTML cache.
            </p>
        </div>
