...
echo $count / $numberCount;</pre>
    ...
    <CodeBlock lang="md"><pre>Write a program that accepts one or more numbers as command-line arguments and prints the mean average of those numbers to the console (stdout).
...
----------------------------------------------------------------------</pre></CodeBlock>
    ...
    <Note type="info">Any instances of <Code>{appname}</Code> will be replaced with the actual application name, this will most likely be the configuration you set when creating your workshop as this is inferred from the command the student executed to run the workshop.</Note>
...
class Mean extends AbstractExercise implements ExerciseInterface, CliExercise
{
    ...
    <p>The <Code>AbstractExercise</Code> class implements a few interesting methods for us. Mainly <Code>getSolution</Code> and <Code>getProblem</Code>. These methods are responsible for locating your solution and problem files. By default they take your exercise's name, normalise it (remove anything that is not A-Za-z or a dash, lowercase and replace spaces with dashes) and look in the <Code>exercises/&lt;normalised-name&gt;/solution</Code> and <Code>exercises/&lt;normalised-name&gt;/problem</Code> folders for files named <Code>solution.php</Code> and <Code>problem.md</Code> respectively. There maybe be cases when you need to override these methods, and in that case you probably don't need to extend from <Code>AbstractExercise</Code>.</p>
    <Note type="info">You may need to override the methods <Code>getSolution</Code> and <Code>getProblem</Code> if you want to organise your problems and solutions in a different structure. We would advise against this in the name of consistency but if you have a good enough reason then the option is there. There may also be the case that your solution is not simply one file. Jump over to <router-link to="/docs/reference/exercise-solutions">Exercise Solutions</router-link> to learn more, if that is the case.</Note>

    <ContentHeader level="h4" id="exercise-types">Exercise Type</ContentHeader>
    <p>Each exercise must have a type, there are currently two types of exercise: <Code>CGI</Code>, <Code>CLI</Code> & <Code>CUSTOM</Code>. Head over to <router-link to="/docs/reference/exercise-types">Exercise Types</router-link> to learn more. We are currently building a
        <Code>CLI</Code> type exercise, this means our reference solution and the student's solution programs will be invoked using the PHP CLI binary. The arguments will come from our exercise class. We inform the workshop of our exercise type by returning an instance of <Code>ExerciseType</Code> from the <Code>getType</Code> method. <Code>ExerciseType</Code> is an ENUM. In conjunction with this, our exercise should implement the respective interface. For <Code>CLI</Code> type exercises this is <Code>CliExercise</Code>.</p>

    <ContentHeader level="h4" id="cli-exercise">CliExercise</ContentHeader>
    <p>This interface defines one method: <Code>getArgs</Code>. This method should return an array of arrays containing string arguments which will be passed to our reference solution and the student's solution at runtime. Each set of arguments will be sent to the solution. So you could essentially run the student's solution as many times as you wanted with different arguments. This method can return random records and random numbers of arguments so that each time the student runs the verification process they receive different arguments. This makes sure the solution is robust.</p>
    ...
    <Note type="info">Do Note that although your implementation of <Code>getArgs</Code> may return random arguments, your reference solution and the student's solution will always receive the same arguments as the <Code>getArgs</Code> method is only called once.</Note>
    ...
    <Note type="success">See the section on <router-link to="/docs/reference/container">The Container</router-link> for more information on service definitions.</Note>
