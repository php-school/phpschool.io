...
<template>
    <p>
        The exercise type indicated how the exercise should be verified, each exercise must have a
        type. There are three types of exercise types as of writing <a href="#cli">CLI</a>,
        <a href="#cgi">CGI</a> & <a href="#custom">CUSTOM</a>. In this section we will detail what
        the verification process for each exercise type is.
    </p>

    <p>
        When selecting your exercise type you must return the valid type and implement the
    ...
    <p>
        ...
        <Code>$argv</Code> and <Code>$argc</Code> super globals.
    </p>
    ...
    <p>
        <Code>getArgs</Code> should return an array of string arrays. These are the arguments to be
        passed to the PHP CLI executable. The number of arguments which can be returned is not
        limited. All arguments should be strings. If multiple sets of arguments are specified then
        the program will be executed each time with those arguments.
    </p>
...
class MyExercise extends AbstractExercise implements ExerciseInterface, CliExercise
{
    ...
    <p>
        ...
        <Code>$_GET</Code>, <Code>$_POST</Code>, <Code>$_SERVER</Code>, etc super globals.
    </p>
    ...
    <p>
        <Code>getRequests</Code> should return an array of PSR-7 Requests. The number of requests is
        not limited. Check <a target="_blank" href="http://www.php-fig.org/psr/psr-7/">here</a> for
        more information about PSR-7. The method is currently type hinted to return an array of the
        <a
            target="_blank"
            href="http://www.php-fig.org/psr/psr-7/#3-2-psr-http-message-requestinterface"
            >Psr\Http\Message\RequestInterface</a
        >
        interface. You will need to use a library which implements the PSR-7 standard. Currently
        <a target="_blank" href="https://github.com/zendframework/zend-diactoros"
            >zendframework/zend-diactoros</a
        >
        is shipped with <Code>php-school/php-workshop</Code> so you can just use that, or feel free
        to pull in a different implementation.
    </p>
...
//$_SERVER
 ...
 array(2) {
    ...
    <p>
        We use the <Code>php-cgi</Code> binary to setup the PHP environment. The
        <Code>php-cgi</Code> binary uses environment variables and standard input to configure
        itself, we export these variables based on the information you provide in
    ...
    <p>
        You choose the CUSTOM type if you want the user to perform some arbitrary task that does not
        involve writing PHP code. For example the exercise could be to ask the user to download and
        install a program and then we could check that it is running.
    </p>
    <p>This exercise type allows to simulate a real HTTP request to a PHP script.</p>
...
class MyExercise extends AbstractExercise implements ExerciseInterface, CustomVerifyingExercise
{
    ...
    <p>
        When a student runs the verify command your verify method will be invoked and the results
