...
<template>
  <p>
    In this article we will build a custom result for our PSR2 check. In the next article, we will build the renderer
    for it, as the result is fairly useless without it.
  </p>

  <p>
    We want to be able to list out each of the coding standard violations as part of the feedback to the student. To do
    this, we will write our result (which is mostly a simple object containing the data) and then we will update our
    check to parse the violations and use the new result class.
  </p>

  <ContentHeader id="getting-started">Getting Started</ContentHeader>
  <p>
    If you are carrying on from the previous article then you can skip this first step of grabbing the tutorial
    workshop.
  </p>
...
class CodingStandardFailure implements FailureInterface
{
  ...
  <p>
    ...
    should return the name of the check this result represents, this is used when rendering the results to the student
    by the workshop framework. This is the only method required by the interface
