...
<template>
    ...
    <Terminal :lines="['cd projects', 'git clone git@github.com:php-school/simple-math.git', 'cd simple-math', 'git checkout custom-interface-check', 'composer install']"></Terminal>

    <ContentHeader level="h4" id="result-step-1">1. Create the folder and class</ContentHeader>
    <Terminal :lines="['mkdir src/Result', 'touch src/Result/CodingStandardFailure.php']"></Terminal>
...
public function check(ExerciseInterface $exercise, Input $input)
{
    ...
    <Note type="danger">If you try to run verify the exercise now, the program will crash as it cannot
        find a renderer for the new result.</Note>
</template>