...
<template>
    ...
    <p>We also described the events features in previous chapters <router-link to="/docs/reference/events">Events</router-link> &
        <router-link to="/docs/reference/creating-listener-checks">Creating Listener Checks</router-link>. Well, as it goes, any events
...
class MyExercise extends AbstractExercise implements ExerciseInterface
{
    ...
    <p>You can grab the <Code>EventDispatcher</Code> from <Code>ExerciseDispatcher</Code> and listen to any events and
        ...
        <router-link to="/docs/reference/creating-listener-checks#build-it">Creating Listener Checks</router-link> when we inserted a verifier
        function which verifies the state of a Couch DB database.</p>

    <Note type="success">You can listen to any of the events listed on <router-link to="/docs/reference/events">Events</router-link>.
    </Note>
</template>