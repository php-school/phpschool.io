...
<template>
    ...
    <p>We learned about <em>Simple Checks</em> in <router-link to="/docs/reference/exercise-checks">Exercise Checks</router-link>, they are simple
        ...
        event is dispatched. Check the <router-link to="/docs/reference/events">Events</router-link> page for a list of available events which
    ...
    <p>Couch DB is a NoSQL database, which stores data as JSON documents and it's API is provided via regular
        HTTP.</p>
...
interface CouchDbExerciseCheck
{
    ...
    <Note type="info">For this check, we assume that Couch DB is always
        running at <Code>http://localhost:5984/</Code>as is default when Couch DB is installed.</Note>
...
class CouchDbCheck implements ListenableCheckInterface
{
    ...
    <CodeBlock lang="php"><pre>$studentClient = CouchDBClient::create(['dbname' => static::$studentDb);
...
$solutionClient->createDatabase($solutionClient->getDatabase());
    ...
    <Note type="danger">We can't just call <Code>seed()</Code> again because <Code>seed()</Code> can return dynamic data and then
    ...
    <CodeBlock lang="php"><pre>$eventDispatcher->listen('verify.start', function (Event $e) use ($studentClient, $solutionClient) {
...
private function replicateDbFromStudentToSolution(CouchDBClient $studentClient, CouchDBClient $solutionClient)
{
    ...
    <CodeBlock lang="php"><pre>$eventDispatcher->listen('run.start', function (Event $e) use ($studentClient) {
    $e->getParameter('exercise')->seed($studentClient);
});
</pre></CodeBlock>
    ...
    <CodeBlock lang="php"><pre>$eventDispatcher->listen('cli.verify.reference-execute.pre', function (CliExecuteEvent $e) {
...
$eventDispatcher->listen(
    ...
    <CodeBlock lang="php"><pre>$eventDispatcher->insertVerifier('verify.finish', function (Event $e) use ($studentClient) {
    ...
    <CodeBlock lang="php"><pre>$eventDispatcher->listen(
    [
        'verify.post.execute',
        'run.finish'
    ],
...
class CouchDbCheck implements ListenableCheckInterface
{
    ...
    <Note type="info">We have to manually require <Code>doctrine/couchdb</Code> even though it is a dependency
    ...
    <CodeBlock lang="md"><pre>Write a program that accepts the name of database and a Couch DB document ID. You should load this document using the
    ...
    CouchDbExerciseCheck
{
    ...
    <Note type="info">Because <Code>seed</Code> is invoked from an event which is dispatched before
...
echo $total;
    ...
    <Note type="info"><Code>composer.lock</Code> is auto generated by Composer, by running
    ...
    <CodeBlock lang="php"><pre>CouchDbExercise::class => object(),
