...
<template>
    <p>In this article we will learn how to create a simple check. If you don't fully understand what checks are, head over
        to the <router-link to="/docs/reference/exercise-checks">Exercise Checks</router-link> article.</p>
    ...
    <p>This method receives an <Code>ExerciseType</Code> instance which represents the type of exercise, we use this to
        inform the workshop which exercise types our check works with: <Code>CLI</Code>, <Code>CGI</Code> or <Code>CUSTOM</Code>. We will support <Code>CLI</Code> & <Code>CGI</Code>.</p>
    ...
    <p>This is the method where we actually perform our check logic, executing PHP_CodeSniffer. This method receives an instance of
        the current exercise and the input arguments passed to our workshop, which will contain the file name of the student's solution.</p>
    ...
    <p>This method needs to return an instance of <Code>PhpSchool\PhpWorkshop\Result\ResultInterface</Code>. Depending on
        ...
        <router-link to="/docs/reference/results">here</router-link>.</p>
    ...
    <CodeBlock lang="php"><pre>//run before checks
...
class Psr2Check implements SimpleCheckInterface
{
    ...
    <Note type="info">As we brought in the tool via Composer, we can rest assured that the binary <code>phpcs</code>
        is available in our projects <Code>vendor</Code> directory.</Note>
    ...
    <p>Now you need to tell the application about your new check. We need to register a factory.
        <router-link to="/docs/reference/container">What's a factory?</router-link> Open up <Code>app/config.php</Code> and add an entry for
...
return $app;
    ...
    <CodeBlock lang="php"><pre>public function configure(ExerciseDispatcher $dispatcher)
{
...
interface Psr2ExerciseCheck
{
    ...
    <CodeBlock lang="php"><pre>public function getExerciseInterface()
{
    ...
    <CodeBlock lang="php"><pre>public function check(ExerciseInterface $exercise, $fileName)
{
    ...
    Psr2ExerciseCheck
{
    ...
    <Note type="success">Maybe you could try updating the check to take into account the standard when returning the
