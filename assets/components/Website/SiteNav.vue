...
<template>
    <nav class="bg-nav-pattern bg-center sm:bg-nav-pattern sm:bg-center sm:bg-[length:110%] px-4 py-2 w-full z-50 relative">
        <div class=" mx-auto flex items-center justify-between ">
            ...
            <div class="hidden sm:flex justify-center items-center">
                <ul class="flex space-x-2 md:space-x-4">
                    <li>
                        <router-link to="/" class="text-white text-xs uppercase hover:text-pink-500 cursor-pointer transition duration-150">Home</router-link>
                    </li>
                    <li>
                        <router-link to="/online" class="text-white text-xs uppercase hover:text-pink-500 cursor-pointer transition duration-150">Workshops</router-link>
                    </li>
                    <li><router-link to="/offline" class="text-white text-xs uppercase hover:text-pink-500 cursor-pointer transition duration-150">Offline</router-link>
                    </li>
                                ...
                                <div class="py-1">
                                    <RouterLink v-slot="{ href, navigate }" to="/docs" custom>
                                        <MenuItem v-slot="{ active, close }">
                                            <a :href="href" @click.prevent="navigate(); close()" :class="[active ? 'bg-gray-700 text-pink-500' : 'text-white', 'block px-4 py-4 text-xs hover:no-underline']">Workshop documentation</a>
                                        </MenuItem>
                                    </RouterLink>

                                    <RouterLink v-slot="{ href, navigate }" to="/submit" custom>
                                        <MenuItem v-slot="{ active, close }">
                                            <a :href="href" @click.prevent="navigate(); close()" :class="[active ? 'bg-gray-700 text-pink-500' : 'text-white', 'block px-4 py-4 text-xs hover:no-underline']">Submit your workshop</a>
                                        </MenuItem>
                                    </RouterLink>
                                </div>
                            </MenuItems>
                    ...
                    <li><router-link to="/events" class="text-white text-xs uppercase hover:text-pink-500 cursor-pointer transition duration-150">Events</router-link></li>
                    <li><router-link to="/blog" class="text-white text-xs uppercase hover:text-pink-500 cursor-pointer transition duration-150">Blog</router-link></li>
                </ul>

                <ul v-if="links" class="flex list-none m-0 ml-4 pl-4 py-1 items-center h-full space-x-4 border-l border-pink-600">
                    <li class="m-0">
...
<div :class="mobileMenuVisible ? 'block fixed inset-0 backdrop-blur bg-white bg-opacity-40 z-30' : 'hidden'"
    class="transition-opacity duration-300" @click="toggleMenu"></div></template>