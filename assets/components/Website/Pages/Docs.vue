...
import Title from "../Docs/Title.vue";
        ...
        <section class="bg-gradient-to-b from-gray-900 to-cyan-500 bg-fixed items-stretch">
            <Heading>
                ...
                <template v-slot:description>
                    ...
                    Use the PHP School workshop documentation to build you own workshop and help teach others how to code with PHP!
                </template>
            </Heading>
        </section>
        <div class="w-full bg-gray-900 min-h-screen">
            <div class=" font-open-sans flex flex-wrap text-left text-gray-800 p-6">
                <div class="w-full md:w-3/12 md:pt-8 border-r border-gray-600 pl-48 bg-gray-900">
                    <div ref="docSearchContainer" class="mr-4 mb-2"></div>
                    <template v-for="group in docs">
                        <h2 class="font-mono text-base pt-4 pb-4 mb-4 border-b border-gray-600 text-white">{{group.title}}</h2>

                        <ul class="">
                            <li v-for="section in group.sections">
                                <router-link :to="sectionRoute(group, section)" exact-active-class='border-r-[3px] border-[#e91e63]' class="block font-open-sans px-3 py-1.5 text-sm text-[#e91e63] hover:underline">
                                    {{section.title}}
                                </router-link>
                            </li>
                        </ul>
...
.docs-content {
    ...
    & > div > p {
        @apply mb-4 text-sm text-gray-300 leading-6;
    }
    ...
    & > h3 {
        @apply pt-2 mb-2 font-mono text-lg;
    }

    & > h4 {
        @apply pt-2 mb-2 font-mono text-base italic;
    }

    & ul {
        @apply list-disc list-outside text-sm my-4 ml-6;
    }
}
...
.DocSearch-Button {
    @apply w-full rounded-md !ring-1 !ring-[#e91e63] pr-3 ml-0 bg-gray-800 hover:bg-gray-700;
}

.DocSearch-Search-Icon {
    @apply !text-[#e91e63] !mr-3 !text-sm;
}
...
.DocSearch-Button-Key {
    @apply font-sans text-xs  p-0 mr-0 !w-auto;
}

.DocSearch-Commands-Key {
    @apply ring-1 ring-[#e91e63] p-0
}
