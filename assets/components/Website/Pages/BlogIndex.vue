...
const { page } = defineProps({'page' : {type: String, default: 1, required: false}});
</script>
<template>
    <div>
        <section class="bg-gradient-to-b from-gray-900 to-cyan-500 bg-fixed items-stretch">
            <Heading>
                <template v-slot:title>
                    What's happening in <br>the PHP School world
                </template>
            </Heading>
        </section>
        <div class="w-full bg-gray-900">
            <div class="font-open-sans flex flex-col mx-auto max-w-3xl text-left text-gray-300 p-8">
                ...
                <div v-for="post in blog.posts" class="p-8 border-b border-solid border-1 border-pink-600/40 text-lg last:border-none">
                    <div class="text-center mb-3">
                        <h1 class="text-4xl font-bold leading-normal mt-[9px] mb-3 mx-0 font-mono text-white hover:text-[#e91e63]"><router-link :to="'/blog/' + post.slug"> {{post.title}}</router-link></h1>
                        <span class="text-xs font-bold hover:text-[#e91e63]"><a :href="post.authorLink">{{post.author}}</a></span>
