...
const verifySuccess = async () => {
    ...
    await studentStore.completeExercise(
        currentExercise.workshop.code,
        currentExercise.exercise.slug
    )
}
...
const resetFiles = async () => {
    const ok = await confirm.value.show({
        title: 'Resetting...',
        message:
            'File tree will be completely reset. All of your code will be deleted. Are you sure you want to continue?',
        okMessage: 'Confirm'
    })
...
const deleteFileOrFolder = async (file) => {
    if (file.name === props.entryPoint && file.parent === null) {
        ...
        confirm.value.show({
            title: 'Error',
            message:
                'Cannot delete the entry point file. This file is required to run your solution.',
            okMessage: 'OK',
            disableCancel: true
        })
        ...
        <div class="h-full relative">
            ...
            <div class="h-full flex flex-col">
                <div class="flex flex-1 h-full relative border-t border-gray-600">
                    ...
                    <div
                        class="flex border-l border-solid border-gray-600 h-full"
                    ...
                    >
                        <div
                            class="pl-4 pr-4 py-4 flex justify-between items-center border-solid border-b border-gray-600"
                        >
                            <h1 class="font-mono text-xl pt-0 flex items-center text-white">
                                <AcademicCapIcon class="h-5 w-5 mr-3" /> Results
                            </h1>
                            <div>
                                <button
                        ...
                        <ResultList
                            :workshop="currentExercise.workshop"
                            :results="results"
                        ></ResultList>
                    </div>
                </div>
                <!-- start footer -->
                <div
                    class="border-t border-solid border-gray-600 h-16 flex-none flex items-center justify-between p-2"
                >
