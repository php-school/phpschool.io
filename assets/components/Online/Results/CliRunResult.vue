...
<template>
    <div class="mt-3 pl-2 border-red-500 border-l-2 border-solid" v-for="(failure, i) in failures" :key="i">
        <div class="w-full flex justify-between">
            <span class="text-sm w-2/3 inline text-gray-300 italic">Run #{{ i + 1 }}</span>
            <button
                class="text-sm ml-2 underline text-[#E91E63] text-left p-x2"
                @click="openInfoModal(failure)"
            >
                Show info
            </button>
        </div>
        <component
            v-if="renderers.hasOwnProperty(failure.type)"
            :is="renderers[failure.type]"
            :data="failure"
        ></component>
    </div>
            ...
            <template #header>
                <div class="flex flex-col">
                    <div class="flex items-center">
                        ...
                        <h2 class="font-mono text-base font-semibold lg:text-xl text-white pt-0 mt-0">
                            Run context
                        </h2>
                    </div>
                </div>
                ...
                <div id="cli-run-info">
                    ...
                    <div v-if="currentFailure.args.length">
                        <div class="mb-8">
                            ...
                            <div v-else>
                                <code v-for="arg in currentFailure.args" :key="arg" class="px-2 mr-2">{{ arg }}</code>
                            </div>
                        </div>
