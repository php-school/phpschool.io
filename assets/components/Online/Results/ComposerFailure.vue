...
<template>
  <div class="mt-1 w-full flex justify-between items-start">
    <p class="text-sm text-red-500 w-2/3 ">Composer requirements were not fulfilled</p>
    <button class="text-sm underline text-[#E91E63] text-left p-x2" @click="openModal = true">More info...</button>
  </div>
      ...
      <template #header>
        <div class="flex flex-col">
          <div class="flex items-center ">
            ...
            <h2 class="font-mono text-base font-semibold lg:text-xl text-white pt-0 mt-0 ">Composer requirements failure...</h2>
          </div>
        </div>
      </template>
      ...
      <template #body class="">
        ...
        <div id="composer-requirements" class="flex space-x-4">
          <div v-if="data.is_missing_component" class="">
            <h2 class="font-mono text-[#E91E63] mb-2 text-lg pt-0">Missing Composer configuration</h2>
            <p class="mb-4 text-sm">The file <code>{{ data.missing_component }}</code> is missing. You must specify some requirements for the file to be created.</p>
          </div>
        </div>

        <div id="composer-requirements" class="flex space-x-4">
          <div v-if="data.is_missing_packages" class="">
            <h2 class="font-mono text-[#E91E63] mb-2 text-lg pt-0">Missing packages</h2>
            <p class="mb-4 text-sm">Some packages were not required which should be used in this exercise:</p>
            <code class="inline-block mr-2 text-xs mb-2" v-for="composerPackage in data.missing_packages">{{ composerPackage }}</code>
          </div>
        </div>
      </template>
