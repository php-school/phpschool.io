...
const selectWorkshop = (workshopCode) => {
    selectedWorkshop.value = workshopStore.workshops.find(
...
<template>
    ...
    <div class="w-full flex h-full overflow-hidden gap-4 mt-10 font-mono">
        <div class="w-1/2 h-full">
            <div
                id="workshops"
                class="flex flex-col items-center justify-center bg-gray-800 rounded-lg shadow"
            >
                <div class="px-4 py-5 sm:px-6 w-full">
                    <h3 class="text-lg leading-6 font-medium text-white pt-[13.5px]">
                        PHP School Workshops
                    </h3>
                ...
                <ul id="workshops-list" class="flex flex-col overflow-hidden">
                    <li
                        v-for="workshop in workshopStore.workshops" :key="workshop.code"
                        @click="selectWorkshop(workshop.code)"
                        :class="{
                            'bg-gray-700':
                                selectedWorkshop && workshop.code === selectedWorkshop.code
                        }"
                        ...
                        <div class="select-none cursor-pointer flex flex-1 items-center p-4">
                            ...
                            <a href="#" class="w-24 text-right flex justify-end">
                                <CheckCircleIcon
                                    ...
                                    class="text-pink-500 h-12 w-12 rounded-full border-2 border-solid border-pink-300"
                                />
                                <ArrowRightCircleIcon
                                    v-else
                                    class="text-pink-200 h-9 w-9 rounded-full border-2 border-solid border-pink-500 !fill-none"
                                />
                            </a>
                        </div>
                    </li>
            ...
            <div
                class="flex items-center justify-center bg-gray-800 rounded-lg shadow mt-20 hover:cursor-pointer"
            >
                <router-link to="/offline" class="flex items-center justify-center">
                    <CommandLineIcon
                        class="h-12 mx-3 w-12 text-gray-900 fill-pink-600"
                    ></CommandLineIcon>
                    <div class="pr-4 py-5 sm:pr-6 flex-1">
                        <h3 class="text-sm leading-6 font-medium text-white pt-[13.5px]">
                            PHP School On The Terminal
                        </h3>
                        <p class="mt-1 max-w-2xl text-xs text-gray-200">
                            An alternative way to complete the PHP School workshops is to download
                            them and run them in your own terminal. Check it out here.
                        </p>
                    </div>
                </router-link>
            </div>
        ...
        <div class="w-1/2 h-full">
            <div v-if="selectedWorkshop === null" class="flex flex-col items-center justify-center">
                <h1 class="text-2xl font-mono text-[#e91e63] pt-[13.5px]">Select a workshop</h1>

                <svg
                    class="-scale-x-100 text-white fill-white"
            ...
            >
                <div class="px-4 py-5 sm:px-6 w-full">
                    <div class="w-full flex flex-col items-center">
                        ...
                        <p class="text-white whitespace-pre tracking-wide font-mono">
                ...
                <ul id="workshop-exercises-list" class="flex flex-col w-full">
                    <exercise-entry
                        ...
                        v-for="exercise in selectedWorkshop.exercises"
                        :key="exercise.slug"
                        :exercise="exercise"
                        :selected-workshop="selectedWorkshop"
                        :student="studentStore.student"
                        :student-state="studentStore.studentState"
                    />
                </ul>
            </div>
        </div>
