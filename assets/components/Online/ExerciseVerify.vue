...
<template>
    ...
    <div class="flex items-center">
        <button
            id="run"
            class="border-[#E91E63] hover:bg-[#E91E63] border-solid border-2 text-white text-sm h-full flex items-center justify-center mt-0 mr-2 px-4 w-36 h-[48px] rounded"
        ...
        <button
            id="verify"
            class="flex items-center justify-center mt-0 px-4 w-36 text-white text-sm h-full rounded bg-gradient-to-r from-pink-600 to-purple-500 hover:bg-[#aa1145] transition-all duration-300 ease-in h-[48px]"
            ...
            <template #body>
                <div class="">
                    <TabGroup v-if="programRunResult && programRunResult.runs.length > 1">
                        <TabList
                            className="flex justify-center border-b border-solid border-gray-600"
                        >
                            <Tab
                                as="template"
                                v-slot="{ selected }"
                                v-for="(run, i) in programRunResult.runs"
                                :key="i"
                            >
                                <button
                                    :class="{
                                        'border-b-2 border-pink-500 py-3 px-4 text-pink-400':
                                            selected,
                                        ' py-3 px-4 text-white': !selected
                                    }"
                                >
                                    Run #{{ i + 1 }}
                                </button>
