...
const saveName = async () => {
    ...
    if (!fileName.value.match(regex)) {
        await fileAlert.value.show({
            title: 'Error',
            message:
                'Files and folder name must contain only alphanumerics, dashes, underscores and spaces. Additionally, files must have an extension.',
            okMessage: 'OK',
            disableCancel: true
        })
        return
...
<template>
    <alert ref="fileAlert" />
    <li
        :class="[model === state.selectedFile ? customStyles?.selectedFileClasses ?? '' : '']"
        class="flex flex-col pl-3 py-2 w-full"
    >
        <div
            @click="selectNode(model)"
        ...
        >
            <div class="flex items-center min-w-0">
                ...
                <span v-show="!isBeingEdited" class="hover:text-white flex-1 truncate text-sm">{{
                    model.name
                }}</span>
                <input
                    ref="name"
                    ...
                    class="bg-gray-700 p-1 rounded-sm"
                    v-model="fileName"
                />
                <span class="ml-2 mr-2 text-sm" v-if="isFolder && hasChildren && !isBeingEdited"
                    >[{{ isOpen ? '-' : '+' }}]</span
                >
            </div>
            <div v-if="showControls" v-show="!isBeingEdited" class="hidden group-hover:flex">
                ...
                />
                <TrashIcon
                    @click.stop="deleteChild(model)"
                    class="mr-2 h-5 w-5 cursor-pointer hover:text-pink-500 fill-none"
                    style="fill: none !important"
                />
            </div>
        </div>
