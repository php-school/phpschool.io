    ...
    actions: {
        ...
        async finishLogin(code, state) {
            ...
            const response = await fetch(
                '/api/online/student/login?' + new URLSearchParams({ code, state })
            )
            const data = await response.json()
        ...
        async logout() {
            await fetch('/api/online/student/logout', {
                method: 'POST'
            })
        ...
        isWorkshopComplete(workshop) {
            ...
            if (this.student.state.workshops[workshop.code] === undefined) {
                return false
            }
        ...
        isExerciseCompleted(workshopCode, exerciseName) {
            ...
            return this.student.state.workshops[workshopCode].completedExercises.includes(
                exerciseName
            )
        },
